# Reimplementation of DispatchSignatureClass, using a Singleton Class


import inspect

# Store a raw_function object, and a reference to the object for dispatching it
# When a DispatchableFunction is called, use the dispatch_source to find the raw
...
class DispatchableFunction:
    ...
    def __init__(self, raw_function, dispatch_source):
    ...
    def __call__(self, *args, **kargs):
        df = self.dispatch_source.get_unique_instance().dispatch(self, *args, **kargs) # find it
...
class DispatchBySignature:
    unique_instance = None   # Rebind only the 1st time get_unique_instance (calling __init__) is called
    

    # Unique instance is typically constructed by the first call to get_unique_instance, which
    #   typically occurs when DispatchBySignature.register is first used to decorate a function
    def __init__(self):
        if DispatchBySignature.unique_instance == None:
            DispatchBySignature.unique_instance = self
            self.dispatch_function_map = {}
        else:
            raise Exception('DispatchBySignature.__init__: Attempted to instantiate DispatchBySignature a second time')    
    

    # Called by register and dispatch (in DispatchableFunction.__call__ )to access the unique
    #   DispatchBySignature object containing the dispatch_function_map      
    @staticmethod
    def get_unique_instance():
        if DispatchBySignature.unique_instance == None:
            DispatchBySignature() # sets unique_instance; OK to return it below
        return DispatchBySignature.unique_instance
    ...
    # Decorate the raw_function by putting its key in the dispatch_function_map; ensure it is unique
    @staticmethod
    def register(raw_function):
        DBT_instance = DispatchBySignature.get_unique_instance()
        df = DispatchableFunction(raw_function, DBT_instance)
        sig_key = DBT_instance.key(raw_function, args=None)
        
        if sig_key in DBT_instance.dispatch_function_map:
            raise TypeError('DispatchBySignature.register: attempt to re-register a signature =',sig_key)
 
        DBT_instance.dispatch_function_map[sig_key] = df
        return df
    ...
    def dispatch(self, df, *args, **kargs):
        raw_function = df.raw_function
        # Try for a direct match (all argument types == signature annotations) and return it
        arg_types        = self.key(raw_function, args=args)
        unique_f_to_call = self.dispatch_function_map.get(arg_types, None)
        ...
        matching = [(param_annots, raw_func)
                       for param_annots, raw_func in self.dispatch_function_map.items()
        ...
        if len(matching) == 1:
            return matching[0][1] # [0] only item [1] function for that item
                ...
                raise TypeError("DispatchBySignature.dispatch: No matching functions located for",arg_types,'in',[a for a  in self.dispatch_function_map])
